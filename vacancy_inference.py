...
def get_slots_model(img,model_slot_dict):
    warped_images = []
    if len(model_slot_dict['marks']) == 0:
        return warped_images
    for i in range(len(model_slot_dict['slots'])):
        con1,x1,y1,dir_x1,dir_y1,typ1= model_slot_dict['marks'][i]
        con2,x2,y2,dir_x2,dir_y2,typ2= model_slot_dict['marks'][i+1]
...
def draw_classification(img,pts,classification):
    ...
    img = cv2.putText(img,classification, p_mid, font, 0.7, (255, 255, 255), 3, cv2.LINE_AA)
    #return img
...
def check_vacancy(img,mat,model):
    ...
    for imag,pts in warped_images:
        imag = img_resize(imag)
        classification = pred_image(model,imag)
        image_classification_points.append([imag,classification,pts])
    return image_classification_points


def vacancy_model_init():
